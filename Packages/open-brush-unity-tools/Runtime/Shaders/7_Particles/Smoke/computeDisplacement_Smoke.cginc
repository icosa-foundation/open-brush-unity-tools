...
void computeDisplacement_float(half3 center_WS, half freq, half time, half d, out half3 disp) {

  disp = half3(0, 0, 0);

  disp = half3(1,0,0) * curlX(center_WS * freq + time, d);
  disp += half3(0,1,0) * curlY(center_WS * freq + time, d);
  disp += half3(0,0,1) * curlZ(center_WS * freq + time, d);
  disp = disp * 5 * 0.1;
}